function Transcript {
    if(!(test-Path ".\logs\Schueler_aufraeumen")){
    mkdir ".\logs\Schueler_aufraeumen"
}
    [string]$transcript = (".\logs\Schueler_aufraeumen\"+(get-date -Format "yyyy-MM-dd-HH-mm-ss")+".log")
...
$source     = "OU=Schueler,OU=SHS,OU=Benutzer,OU=SRHSchulenGmbH,OU=_Schulen,DC=srhk,DC=srh,DC=de" 
...
#Alte Logs löschen
$Source = ".\logs\Schueler_aufraeumen\"		# Wichtig: muss mit "\" enden
